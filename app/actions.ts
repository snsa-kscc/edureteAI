...
function saveChat(aiState: ServerMessage[]) {
  console.log("saving chat aiState", aiState);
}
...
export async function submitUserMessage(input: string): Promise<ClientMessage> {
  ...
  const result = await streamUI({
    ...
    text: ({ content, done }) => {
      if (done) {
        aiState.done((messages: ServerMessage[]) => [...messages, { role: "user", content: input }, { role: "assistant", content }]);
      }
...
export const AI = createAI<ServerMessage[], ClientMessage[]>({
  ...
  onGetUIState: async () => {
    const aiState: ServerMessage[] = getAIState();

    return aiState.map(({ role, content }) => ({
      id: randomUUID(),
      role,
      display: content,
    }));
